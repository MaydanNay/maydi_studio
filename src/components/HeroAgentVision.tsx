...
> = {
  ...
  'h1.title': {
    label: 'заголовок',
    verdict:
      '«Simulate reality before it happens» — сильный хук. Слово reality без цифры, срока или P2B в первые 3 секунды.',
    status: 'warn',
  },
  'p.lead': {
    label: 'подзаголовок',
...
export function HeroAgentVision() {
  ...
  useEffect(() => {
    const onMsg = (event: MessageEvent) => {
      ...
      if (isRegionId(event.data.id)) setActive(event.data.id)
    }
